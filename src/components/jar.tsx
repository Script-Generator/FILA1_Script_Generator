...
const Jar = () => {
    ...
    const [files, setFiles] = useState<{ name: string, file: File | null, defaultArgs: string, multiValueArgs: Array<{ paramName: string; values: string[] }> }[]>([]);
    ...
    const handleMultiArgsParamsChange = (fileIndex: number, paramIndex: number) => (event: React.ChangeEvent<HTMLInputElement>) => {
        const newFiles = [...files];
    ...
    const handleMultiArgsValueChange = (fileIndex: number, paramIndex: number) => (event: React.ChangeEvent<HTMLInputElement>) => {
        const newFiles = [...files];
                    ...
                    <div className="flex gap-6 mb-2 items-center">
                        <div className="flex items-center">
                            <Label>
                                Selected file :
                                <span className="ml-3">
                                        <TagComponent file={fileObj.file} onRemove={() => handleRemoveFile(fileIndex)}
                                                      icon="📄" />
                                    </span>
                            </Label>
                        </div>
                    ...
                    {fileObj.multiValueArgs.map((multiArg, paramIndex) => (
                      <div key={paramIndex} className="flex w-full gap-6 items-end">
                          ...
                          <div className="flex flex-col w-2/3 space-y-1">
                              <Label htmlFor={`multiArgsValue-${fileIndex}-${paramIndex}`}>Multi Args Value (Separator
                                  ;)</Label>
                              <Input
